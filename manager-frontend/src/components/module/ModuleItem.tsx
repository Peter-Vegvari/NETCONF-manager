...
export function ModuleItemLabel({ module: m }: { module: ModuleSummary }) {
  return <span>{m.name} <Tag color={statusColor[m.status]}>{m.status}</Tag></span>;
}

export function ModuleItemActions({ module: m, onDownload, onDelete, downloadPending, downloadingName }: Props) {
  if (m.status === "remote") {
    return (
  ...
  return (
    <Popconfirm title="Delete this module?" onConfirm={() => onDelete(m.name)} onPopupClick={(e) => e.stopPropagation()}>
